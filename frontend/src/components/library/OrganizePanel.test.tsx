...
describe('OrganizePanel', () => {
  it('previews selectable destinations and confirms a safe move', async () => {
    ...
    await waitFor(() => expect(screen.getByText('姜育恒/再回首-姜育恒.flac')).toBeInTheDocument());
    expect(screen.queryByText('姜育恒/多年以后·再回首/再回首-姜育恒.flac')).not.toBeInTheDocument();
    await user.click(screen.getByRole('button', {name: /确认整理/}));
    expect(onApply).toHaveBeenCalledOnce();
    expect(onApply).toHaveBeenCalledWith('artist');
  });
});
