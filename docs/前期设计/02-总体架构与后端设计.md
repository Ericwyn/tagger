...
- 首次启动 setup token。
- Argon2id 密码哈希。
- 服务端 session，cookie 为 HttpOnly、SameSite=Lax；HTTPS 时 Secure。
- 写接口校验 Origin/Host 和 CSRF token。
- 登录、写入和 provider 配置设置限速。
- 反向代理头只有在配置 trusted proxy 后才接受。

将来如需多用户，可在领域层加入角色和 library ACL；当前不把复杂权限散布到每个 repository。
